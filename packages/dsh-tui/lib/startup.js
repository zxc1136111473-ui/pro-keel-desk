...
*/
function tuiCommand() {
	return new Command().name("dsh --profile tui").description("启动交互式终端对话；看到 dsh> 后打字，/quit 退出。").helpOption("-h, --help", "显示帮助").option("--session <id>", "会话 id（续用或新建）").option("-e, --task <task>", "启动后先跑一条任务，再进入对话").option("--provider <name>", "指定接口商（空则用默认）").option("--model <id>", "指定模型（空则用默认）").option("--check", "打印已加载插件和工具后退出").addHelpText("after", `
