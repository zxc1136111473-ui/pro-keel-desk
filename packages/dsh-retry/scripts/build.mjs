// Build the dsh-retry plugin: copy the Host ESM verbatim and bundle the browser
// settings row into the Harness lazy-CJS handoff. Mirrors dsh-attachments.
...
if (!output.includes(`id: "${clientId}"`) || !output.includes('factory: (require)')) {
  throw new Error('dsh-retry does not implement the Harness lazy-CJS handoff')
}

console.log(`[dsh-retry] Built Host and browser bundles in ${outDir}`)
